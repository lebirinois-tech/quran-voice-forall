...
// ~5 MB of base64 audio (≈ 3.75 MB binary)
const MAX_AUDIO_BASE64_LEN = 7_000_000;
const MAX_VERSE_TEXT_LEN = 4_000;
...
serve(async (req) => {
  ...
  try {
    ...
    // ---- Input validation ----
    if (typeof audioBase64 !== "string" || typeof verseText !== "string" || !audioBase64 || !verseText) {
      return new Response(JSON.stringify({ error: "Missing or invalid audio or verse text" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (verseText.length > MAX_VERSE_TEXT_LEN) {
      return new Response(JSON.stringify({ error: "Verse text too long" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!Number.isInteger(surah) || surah < 1 || surah > 114 ||
      ...
      return new Response(JSON.stringify({ error: "Invalid surah or verse number" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
